/**
 * 고급 프롬프트 템플릿으로 데이터베이스 업그레이드 스크립트
 * 
 * 기존 단순한 템플릿을 STICK 원칙, HSO 프레임워크, 
 * Anti-AI 탐지 기능을 포함한 고급 템플릿으로 교체
 */
...
// 템플릿 이름 매핑 (기존 db-prompt-templates.ts와 일치)
...
async function upgradePromptTemplates() {
  try {
    console.log('🚀 고급 프롬프트 템플릿 업그레이드 시작...')
    
    const supabase = await createClient()
    
    // 1. 콘텐츠 생성 카테고리 ID 가져오기
    const { data: category, error: categoryError } = await supabase
      .from('prompt_categories')
      .select('id')
      .eq('name', 'content_generation')
      .single()
    
    if (categoryError || !category) {
      throw new Error('콘텐츠 생성 카테고리를 찾을 수 없습니다.')
    }

    const categoryId = category.id
    console.log('✅ 카테고리 ID 확인:', categoryId)

    // 2. 각 콘텐츠 타입별로 v2 템플릿 생성
    for (const [contentType, templateData] of Object.entries(ADVANCED_TEMPLATES)) {
      console.log(`\n📝 ${contentType} 템플릿 업그레이드 중...`)
      ...
      if (deactivateError) {
        console.warn(`⚠️ 기존 ${templateData.name} 템플릿 비활성화 실패:`, deactivateError)
      } else {
        console.log(`✅ 기존 ${templateData.name} 템플릿 비활성화 완료`)
      }
      ...
      const { data: newTemplate, error: insertError } = await supabase
        ...
        .insert({
          ...
          is_active: true,
          is_default: true,
          change_notes: 'STICK 원칙, HSO 프레임워크, Anti-AI 탐지 기능을 포함한 고급 템플릿으로 업그레이드',
      ...
      if (insertError) {
        console.error(`❌ ${templateData.name} 생성 실패:`, insertError)
        continue
      }

      console.log(`✅ ${templateData.name} v2 템플릿 생성 완료!`)
      console.log(`   - ID: ${newTemplate.id}`)
      console.log(`   - 변수: ${templateData.variables.join(', ')}`)
      console.log(`   - 길이: ${templateData.template.length}자`)
    }

    console.log('\n🎉 모든 프롬프트 템플릿 업그레이드 완료!')
    console.log('\n📋 업그레이드된 템플릿 목록:')
    
    // 업그레이드 결과 확인
    const { data: upgradedTemplates, error: checkError } = await supabase
      .from('system_prompt_templates')
      .select('name, title, version, is_active, created_at')
      .eq('version', 2)
      .eq('is_active', true)
      .order('name')

    if (checkError) {
      console.error('❌ 업그레이드 결과 확인 실패:', checkError)
    } else {
      upgradedTemplates?.forEach((template) => {
        console.log(`   ✓ ${template.name} v${template.version} (${template.is_active ? '활성' : '비활성'})`)
      })
      
      console.log(`\n📊 총 ${upgradedTemplates?.length || 0}개 템플릿이 성공적으로 업그레이드되었습니다!`)
    }

    // 업그레이드 완료 후 권장사항
    console.log('\n💡 다음 단계 권장사항:')
    console.log('1. /content/generate 페이지에서 새 템플릿 테스트')
    console.log('2. 관리자 페이지에서 템플릿 성능 모니터링')
    console.log('3. 사용자 피드백 수집 및 반영')
    console.log('4. A/B 테스트로 기존 vs 새 템플릿 성과 비교')

  } catch (error: any) {
    console.error('❌ 프롬프트 템플릿 업그레이드 실패:', error)
    process.exit(1)
  }
}
...
export { upgradePromptTemplates, ADVANCED_TEMPLATES }